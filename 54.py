...
class AppLauncher:
    ...
    def call_program(self, name):
        ...
        # Якщо запис є рядком, це файл (без аргументів)
        if isinstance(entry, str):
            ...
            try:
                if platform.system() == "Windows":
                    ...
                    subprocess.run(["xdg-open", path] if platform.system() == "Linux" else ["open", path])
            except Exception as e:
                print(self.loc._("error_with_start").format(name=name, e=e))

        # Якщо запис є списком, перший елемент - це файл, решта - аргументи
            ...
            if not os.path.isabs(path):
                path = os.path.abspath(path)
            try:
                # Перевірка наявності "start_with_python"
                if path.endswith(".py") and "start_with_python" in args:
                    args.remove("start_with_python")
    ...
    def update_process_info(self, pid, name, cpu, memory, status, operation, children_info):
        ...
        try:
            if self.info_window_live:
                ...
                for child_info in children_info:
                    self.treeview.insert("", "end", text=child_info["Name"], values=(child_info["Name"], child_info["PID"], child_info["CPU"], child_info["Memory"], child_info["Status"], child_info["Operation"]), tags=("grandchild_process"))
                self.treeview.tag_configure("grandchild_process", foreground="blue")
        except:
            self.info_window_live = False
